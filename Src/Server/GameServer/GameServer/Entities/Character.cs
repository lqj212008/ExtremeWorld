...
namespace GameServer.Entities
{
    class Character : CharacterBase,IPostResponse
    {
        ...
        public void PostProcess(NetMessageResponse response)
        {
            ...
            if (this.Guild != null)
            {
                Log.InfoFormat("PostProcess > Guild: characterID:{0}:{1} {2}<{3}", this.Id, this, GuildUpdateTS, this.Guild.timestamp);
                ...
                if (GuildUpdateTS < this.Guild.timestamp && response.mapCharacterEnter == null)
                {
                    GuildUpdateTS = Guild.timestamp;
